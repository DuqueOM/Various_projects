...
# === CONFIG ===
INPUT_FILE = "BANJIO.csv"
OUTPUT_FILE = "extractos_limpios.csv"
...
raw_rules = [
    ...
    (r'^\s*devoluci[oó]n\s+de\s+spei[:\s]?.*',
     "Devolución de SPEI", 'Devolución de SPEI'),
    (r'^\s*entrega\s+de\s+recursos\s+por',
     "Entrega de Recursos", 'Entrega de Recursos'),
    (r'^\s*retiro\s+de\s+recursos\s+por',
     "Retiro de Recursos", 'Retiro de Recursos'),
    ...
    (r'telefono-?telmex', "Pago de Servicios TELMEX EN LINEA",
     'Pago de Servicios TELMEX EN LINEA'),
    (r'n[oó]mina\b', "Nómina", 'Nómina'),

    (r'por\s*\(?[\d,]+\.\d{2}\)?\s*mxn\s*\|\s*recibo\s*#\s*\d+\b',
     "por *", "por (monto) mxn | Recibo # (caso general estricto)"),

    # 🚩 ESTA ES LA QUE CAMBIA
    (r'por\s*\(?[\d,]+\.\d{2}\)?\s*mxn\s*cargo',
        "__RAW__por_mxn_cargo", 'por (xx) mxn Cargo'),
]

compiled_rules = [(re.compile(pat, re.IGNORECASE | re.UNICODE), label, debug)
...
def apply_rules_vectorized(df: pd.DataFrame, text_col: str):
    ...
    for pattern, label, debug in compiled_rules:
        ...
        if has_group:
            extracted = df.loc[mask_unassigned,
                               raw_col].str.extract(pattern, expand=True)
            if extracted.dropna(how="all").empty:
                extracted = df.loc[mask_unassigned,
                                   norm_col].str.extract(pattern, expand=True)
            ...
            if not idx.empty:
                ...
                def build_label(g):
                    ...
                    if label.lower().startswith("spei enviado") or label.lower().startswith("spei recibido"):
                        return f'{label}: {gstr}'
            ...
            if label in RAW_LABEL_SET:
                ...
                if not idx.empty:
                    ...
                    if label == "__RAW__por_mxn_cargo":
                        # 🚩 limpieza especial: quitar "Cargo a cuenta..." hasta el primer "|"
...
def main():
    ...
    if DESCRIPTION_COL not in df.columns:
        raise ValueError(
            f"La columna '{DESCRIPTION_COL}' no existe en el archivo CSV")
